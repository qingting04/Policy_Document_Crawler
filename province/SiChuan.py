...
def get_url(policy):
    url = f'https://www.sc.gov.cn/search/s?siteCode=5100000062&tab=zcwj&qt={policy}'
    driver = initialize_driver()
    try:
        driver.get(url)
        wait = WebDriverWait(driver, 5)
        wait.until(EC.presence_of_element_located((By.CSS_SELECTOR, '.load-container[style="display: none;"]')))
        ...
        while True:
            ...
            for elements in poli:
                line = elements.find_element(By.CLASS_NAME, 'source-text').text
            ...
            try:
                driver.find_element(By.CSS_SELECTOR, '.m-style.M-box[style="display: none;"]')
                break
...
def get_content(data_process):
    ...
    xpath = "//*[@class='contText'] | //*[@id='cmsArticleContent']"
    try:
        count = 0
...
def main(un_policy):
    ...
    print(f"四川共计{total}篇文章")
    data = get_content(data_process)
    #mysql_writer('sichuan_wj', data)
